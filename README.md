...
Kubernetes namesapces (`kube-system`, `kube-node-lease` and `kube-public`) will be patched with the label when you deploy the module. The KFD *infrastructure* namespaces are created outside of this module, so to completely exempt one of them, you need to label it with the following command:
